...
import { Search, ChevronLeft, Clock, Send, Lamp, List, CheckCircle } from 'lucide-react';
import { ExamResult } from '../App';
import { OFFICIAL_HOC_LIEU_TEST, HocLieuTestData, Question } from '../data/hocLieuData';
import { mistakeStore } from '../data/mistakeStore';

interface HocLieuExamProps {
  testId: number;
  onBack: () => void;
  onLogout: () => void;
...
const HocLieuExam: React.FC<HocLieuExamProps> = ({ testId, onBack, onLogout, onFinish }) => {
  const testData = OFFICIAL_HOC_LIEU_TEST;
  const [secondsLeft, setSecondsLeft] = useState(testData.estimatedTime * 60);
  const [answers, setAnswers] = useState<Record<number, string>>({});
  const [activeQuestionId, setActiveQuestionId] = useState<number>(1);

  // Flatten questions for sidebar navigation
  ...
  const handleSubmit = useCallback(() => {
    ...
    allQuestions.forEach(q => {
      ...
      if (userAns === q.correct) {
        ...
        mistakesToSave.push({
          questionId: q.id,
          stem: q.label, // Fallback for hoc lieu which uses label as stem mostly
          passage: section?.passage,
          options: q.options,
          correctAnswer: q.correct,
          userAnswer: userAns || null,
          explanation: q.explanation,
          topic: section?.title || 'Học liệu',
        ...
        <main className="flex-grow flex flex-col overflow-y-auto bg-slate-50/50 scroll-smooth">
           <div className="p-4 lg:p-12 max-w-5xl mx-auto w-full space-y-16 pb-32">
              <div className="text-center mb-16">
                 ...
                 <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">Phần học liệu độc quyền ENGMIND 2026</p>
              </div>
              ...
              {testData.sections.map((section) => (
                <div key={section.id} className="bg-white rounded-[32px] border border-slate-100 p-8 md:p-12 shadow-sm space-y-10">
                   ...
                   <div className="bg-[#f8fafc] rounded-2xl p-6 md:p-10 border border-slate-100 text-slate-600 text-lg leading-[1.8] whitespace-pre-wrap select-none">
                      {section.passage}
                   </div>
